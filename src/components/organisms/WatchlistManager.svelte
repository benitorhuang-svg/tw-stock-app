<script lang="ts">
    ...
    const NAV_ORDER = ['guide', 'watchlist', 'yield'];
...
<div class="flex flex-col lg:flex-row gap-4 items-start relative pb-10 pt-4">
            ...
            <div class="flex flex-col gap-2 mt-2">
                {#if stocks.length > 0}
                    ...
                    <div class="flex items-center justify-between">
                        ...
                        <span class="text-[10px] font-black font-mono text-bearish"
                            >{bearCount}</span
                        >
                    </div>
                {:else}
    ...
    <main class="flex-1 space-y-4 animate-fade-right min-w-0">
        ...
        >
            ...
            >
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {#each [{ icon: '🎯', title: '追蹤矩陣', desc: '您加入自選清單的個股即時價格、漲跌幅概覽。點擊個股卡片可進入深度分析頁面。' }, { icon: '💰', title: '高殖利率', desc: '全市場殖利率 Top 10 排行，包含即時價格、本益比、股價淨值比等關鍵指標。' }, { icon: '📌', title: '如何加入', desc: '在個股分析頁面點擊「加入自選」即可追蹤，清單自動同步至此頁面。' }] as item}
        ...
        <AnalysisAccordion
            id="watchlist"
            icon="🎯"
            title="追蹤矩陣 ( WATCHLIST MATRIX )"
            isOpen={expandedSections.watchlist}
            onToggle={() => toggleSection('watchlist')}
        >
            {#if !isLoading && stocks.length === 0}
                <div class="glass-card p-12 text-center border border-border/50">
                    <h3
                        class="text-xs font-mono font-black text-text-muted/70 uppercase tracking-[0.3em]"
                    >
                        無追蹤標的 / NO_ACTIVE_LINKS
                    </h3>
                    <p
                        class="text-[10px] font-mono text-text-muted/50 uppercase tracking-[0.3em] mt-2"
                    >
            ...
            {:else if isLoading}
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 animate-pulse">
                    {#each Array(3) as _}
                        <div
                            class="h-32 bg-surface-hover rounded-2xl border border-border/50"
                        ></div>
                    {/each}
                </div>
            {:else}
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {#each stocks as s}
                        ...
                        <a
                            ...
                            class="glass-card relative p-6 hover:bg-surface-hover transition-all group no-underline overflow-hidden border border-border/50 hover:border-border"
                        >
                            <div
                            ...
                            <div class="flex items-start justify-between mb-4">
                                <div class="min-w-0">
                                    <div
                                        class="text-sm font-black text-text-primary group-hover:text-accent transition-colors truncate"
                                    >
                                        {s.name}
                                    </div>
                                    <div
                                        class="text-[10px] font-mono text-text-muted/70 uppercase tracking-widest mt-0.5"
                                    >
                                        {s.symbol}
                                    </div>
                                </div>
                            ...
                            <div
                                class="text-2xl font-mono font-black text-text-primary tracking-tighter mb-4"
                            >
                                {s.price.toFixed(2)}
                            </div>
                            <div
                                class="flex items-center justify-between pt-4 border-t border-border/50 opacity-40 group-hover:opacity-100 transition-opacity"
                            >
        ...
        <AnalysisAccordion
            id="yield"
            icon="💰"
            title="高殖利率監控 ( YIELD QUANTUM )"
            isOpen={expandedSections.yield}
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            ...
                            >
                                ...
                                <th class="py-4 px-8 border-b border-border/50 text-right"
                                    >Yield %</th
                                >
                            ...
                            {#each highYield as s}
                                ...
                                >
                                    <td class="py-5 px-8">
                                        <div class="flex flex-col">
                                            <span
                                                class="text-[11px] font-black text-text-primary group-hover:text-warning transition-colors"
                                                >{s.name}</span
                                            >
                                            <span
                                                class="text-[9px] font-mono text-text-muted/50 uppercase tracking-widest mt-0.5"
                                                >{s.symbol}</span
                                            >
                                        </div>
                                    </td>
                                    <td
                                        ...
                                        >{s.yield.toFixed(2)}%</td
                                    >
                                    <td
                                        class="py-5 px-8 text-right font-mono text-xs font-bold text-text-muted"
                                        >{s.price.toFixed(2)}</td
                                    >
                                    <td
                                        class="py-5 px-8 text-right font-mono text-[10px] text-text-muted/70 tracking-widest uppercase"
                                        >{s.pe.toFixed(1)} / {s.pb.toFixed(2)}</td
                                    >
                                </tr>
                            {/each}
                        </tbody>
