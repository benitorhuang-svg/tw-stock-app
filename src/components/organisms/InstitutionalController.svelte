...
<div class="flex flex-col lg:flex-row gap-4 items-start relative pb-10 pt-4">
    ...
    >
        ...
        <div class="px-5 py-6 flex flex-col gap-4 relative border-b border-border/50">
            <div class="flex items-center gap-3">
                ...
                <div class="flex flex-col">
                    <span class="text-[11px] font-black text-text-primary/80 tracking-wide uppercase">法人監控</span>
        ...
        <div class="flex-1 overflow-y-auto [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            <div class="px-5 flex-1 py-1 flex flex-col gap-0.5 bg-surface/5">
                <span class="text-[10px] font-mono font-black text-text-muted/40 uppercase tracking-[0.25em] mb-1">
                    快速索引 ( NAV )
                </span>
                {#each navItems as nav}
                    ...
                    >
                        <span class="text-base {isActive ? 'opacity-100 scale-110 drop-shadow-sm' : 'opacity-30 group-hover:opacity-100'} transition-all duration-300">
                            {nav.icon}
                        </span>
                        <span class="text-xs font-bold tracking-widest {isActive ? 'text-accent' : 'text-text-muted/60 group-hover:text-text-primary'} transition-colors">
                            {nav.label}
                        </span>
                        {#if isActive}
                            <div class="ml-auto w-1.5 h-1.5 rounded-full bg-accent shadow-[0_0_12px_rgba(0,184,212,1)]"></div>
                            <div class="absolute left-0 top-1.5 bottom-1.5 w-[3px] bg-accent rounded-r-full transition-all shadow-[0_0_10px_rgba(0,184,212,0.6)]"></div>
                        {/if}
                    </button>
                {/each}
            </div>
    ...
    <main class="flex-1 space-y-4 animate-fade-right min-w-0">
        ...
        <AnalysisAccordion id="guide" icon="📖" title="功能說明 ( FEATURE GUIDE )" isOpen={expandedSections.guide} onToggle={() => toggleSection('guide')}>
            <div class="glass-card bg-base-deep/30 px-5 py-4 shadow-elevated rounded-xl border border-border/10">
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                    {#each [
                        { icon: '📡', title: '法人總覽', desc: '顯示外資、投信、自營商當日買賣超金額總覽，官股動向及市場集中度等關鍵指標。搜尋、市場別、背離篩選一覽無遺。' },
                        { icon: '🏛️', title: '三法人監控', desc: '外資、投信、自營商分三欄獨立顯示，各欄呈現連續買賣超天數、淨買賣張數、漲跌背離訊號等資訊。' },
                        { icon: '📊', title: '統一列表', desc: '將三大法人操作整合至同一張表格，同一檔股票可同時對比外資/投信/自營商的買賣操作，支援多欄排序。' },
                        { icon: '🔬', title: '籌碼雷達', desc: '進階篩選：高集中度股、官股買超、主力券商佈局、內部人異動、融資融券等五大面向，快速鎖定籌碼異常標的。' },
                        { icon: '📋', title: '異常報告', desc: '自動偵測法人行為異常模式：急買急賣、連續買超反轉、量能暴增等訊號，並產出結構化報告。' },
                        { icon: '📈', title: '趨勢分析', desc: '20日法人買賣超趨勢圖，疊加市場均價走勢，觀察法人資金流向與股價的領先/落後關係。' },
                    ] as item}
        ...
        <AnalysisAccordion id="pulse" icon="📡" title="法人總覽 ( INSTITUTIONAL PULSE )" isOpen={expandedSections.pulse} onToggle={() => toggleSection('pulse')}>
            <InstitutionalPulseHUD />
        </AnalysisAccordion>

        <!-- ② 三法人監控 ( MONITORING MATRIX ) -->
        <AnalysisAccordion id="matrix" icon="🏛️" title="三法人監控 ( MONITORING MATRIX )" isOpen={expandedSections.matrix} onToggle={() => toggleSection('matrix')}>
            <InstitutionalMatrix />
        </AnalysisAccordion>

        <!-- ③ 統一列表 ( UNIFIED FLOW TABLE ) -->
        <AnalysisAccordion id="unified" icon="📊" title="統一列表 ( UNIFIED FLOW TABLE )" isOpen={expandedSections.unified} onToggle={() => toggleSection('unified')}>
            <InstitutionalUnifiedTable />
        </AnalysisAccordion>

        <!-- ④ 籌碼雷達 ( FORENSIC RADAR ) -->
        <AnalysisAccordion id="radar" icon="🔬" title="籌碼雷達 ( FORENSIC RADAR )" isOpen={expandedSections.radar} onToggle={() => toggleSection('radar')}>
            <ForensicRadar />
        </AnalysisAccordion>

        <!-- ⑤ 異常報告 ( FORENSIC REPORT ) -->
        <AnalysisAccordion id="report" icon="📋" title="異常報告 ( FORENSIC REPORT )" isOpen={expandedSections.report} onToggle={() => toggleSection('report')}>
            <ForensicReport />
        </AnalysisAccordion>

        <!-- ⑥ 趨勢分析 ( VELOCITY ANALYTICS ) -->
        <AnalysisAccordion id="analytics" icon="📈" title="趨勢分析 ( VELOCITY ANALYTICS )" isOpen={expandedSections.analytics} onToggle={() => toggleSection('analytics')}>
