<script lang="ts">
    ...
    // Auto-expand first item whenever filtered list changes or none expanded
    $effect(() => {
        const first = filteredAndSorted[0];
        if (first && (!expandedCode || !filteredAndSorted.some(s => s.code === expandedCode))) {
            expandedCode = first.code;
        }
    });
    ...
    <table class="w-full text-left table-fixed whitespace-nowrap border-separate border-spacing-0">
        ...
        <thead class="z-30">
            <tr>
                {#each headers as h}
                    <th
                        class="atom-th sticky z-20 bg-surface select-none transition-colors"
                        class:cursor-pointer={!!h.col}
                        class:hover:text-accent={!!h.col}
                        style="top: var(--toolbar-nexus-height, 0px); text-align: center;"
                ...
                {#each displayItems as s (s.code)}
                    ...
                    >
                        ...
                        <td class="px-1.5 py-2 text-center">
                            <div
                                class="atom-badge min-w-[64px] h-[22px] tracking-tighter"
                                class:atom-badge-bull={isUp}
                                class:atom-badge-bear={isDown}
                                class:atom-badge-flat={!isUp && !isDown}
                            >
                                {(isUp ? '+' : '') +
