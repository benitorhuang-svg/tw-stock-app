...
describe('Database Module', () => {
    ...
    describe('execute', () => {
        it('應執行 SQL 並返回影響的行數', async () => {
            ...
            const rowsModified = await execute(
...
describe('Schema', () => {
    it('應包含 stocks 表格', async () => {
        ...
        const hasStocksTable = runCalls.some(
            (call: unknown[]) => (call[0] as string)?.includes('CREATE TABLE IF NOT EXISTS stocks')
        );
    ...
    it('應包含 daily_prices 表格', async () => {
        ...
        const runCalls = dbMock.run.mock.calls;
        const hasDailyPricesTable = runCalls.some(
            (call: unknown[]) => (call[0] as string)?.includes('CREATE TABLE IF NOT EXISTS daily_prices')
        );
    ...
    it('應包含 portfolio 表格', async () => {
        ...
        const runCalls = dbMock.run.mock.calls;
        const hasPortfolioTable = runCalls.some(
            (call: unknown[]) => (call[0] as string)?.includes('CREATE TABLE IF NOT EXISTS portfolio')
        );
