...
| Transformer、ML、深度學習、模型、預測 | `transformer_strategy` | ML 選股 |
| 交易記憶、反思、L1/L2/L3、策略調整 | `transformer_strategy` | Trade Memory |
...
- **「ML 選股」** → `transformer_strategy` (+ `backtest_engine` 驗證)
- **「模型訓練」** → `transformer_strategy`
- **「策略反思」** → `transformer_strategy`
...
所有模型共用 `stocks.db` (SQLite)，5 層 31 表，schema 定義於 `scripts/db/schema.sql`。
...
- **ML 記憶層 (4 表)**：`trade_journal`, `trade_patterns`, `strategy_adjustments`, `ml_models`
